/* =========================
   NeoBank Demo + Chatbot
   Safe version for GitHub Pages
   ========================= */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const initialState = () => ({
    isLoggedIn: false,
    balances: { savings: 75000, checking: 42000 },
    transactions: [
  ...
  function wireEvents() {
    ...
    resetBtn?.addEventListener("click", () => {
      ...
      closeLogin();
      renderAll();
      flashSuccess("Demo reset done.");
    });

    // ===== Chatbot Toggle Events =====
  ...
  function handleLogin() {
    ...
    if (!ok) {
      if (loginError) loginError.textContent = "Invalid credentials. Use demo@neobank.com / demo123";
      return;
    }

    state.isLoggedIn = true;
    if (userNameEl) userNameEl.textContent = DEMO_USER.name;
  ...
  function handleTransfer() {
    ...
    if (!from || !to) return flashError("Select From/To accounts.");
  ...
  function renderLanding() {
    if (!landingTotal || !landingSavings || !landingChecking) return;
